...
export const PaymentsListView: React.FC<PaymentsListViewProps> = ({
  ...
  const { transactions, parties, deleteTransaction, openPrintModal, cashAccounts } = useAccounting();
  ...
  const payments = useMemo(() => {
    return transactions.filter(t => t.type === 'make_payment');
  }, [transactions]);
  ...
  }, [payments, currencyFilter, selectedCashAccount, selectedPartyId, startDate, endDate, minAmount, maxAmount, searchQuery]);
  ...
  const handleDelete = (id: string, num: string) => {
    if (window.confirm(`آیا از حذف سند پرداخت شماره ${num} اطمینان دارید؟ موجودی صندوق و مانده حساب فروشنده به حالت قبل برمی‌گردد.`)) {
  ...
  const handlePrintReport = () => {
    openPrintModal({
      type: 'report',
      title: 'گزارش رسمی و جامع اسناد پرداخت صندوق (خروجی نقدینگی)',
      subtitle: `فیلتر شده بر اساس معیارهای جستجو • تعداد اسناد: ${filteredPayments.length} عدد • تاریخ تهیه: ${getPersianDate()}`,
      customContent: (
        <div className="space-y-4 text-slate-800">
          <div className="grid grid-cols-3 gap-3 p-3 bg-rose-50 rounded-xl border border-rose-200 text-xs">
            <div>
              <span className="text-slate-500 block">تعداد اسناد پرداخت:</span>
              <strong className="text-sm font-black text-slate-900">{metrics.count} سند</strong>
            </div>
            <div>
              <span className="text-slate-500 block">مجموع پرداختی افغانی:</span>
              <strong className="text-sm font-black text-rose-800 font-mono">{formatNumber(metrics.totalAFN)} ؋</strong>
            </div>
            <div>
              <span className="text-slate-500 block">مجموع پرداختی دالری:</span>
              <strong className="text-sm font-black text-rose-800 font-mono">${formatNumber(metrics.totalUSD)}</strong>
            </div>
          </div>
          ...
          <table className="w-full text-right border-collapse text-xs">
            <thead>
              <tr className="bg-slate-900 text-white font-bold text-[11px]">
                ...
                <th className="p-2 border border-slate-700 text-center">شماره سند</th>
                <th className="p-2 border border-slate-700 text-center">تاریخ</th>
                <th className="p-2 border border-slate-700">طرف حساب (دریافت‌کننده وجه)</th>
                <th className="p-2 border border-slate-700">شرح پرداخت</th>
                <th className="p-2 border border-slate-700 text-center">صندوق پرداخت</th>
                <th className="p-2 border border-slate-700 text-center">مبلغ پرداختی</th>
              </tr>
            </thead>
            <tbody>
              {filteredPayments.map((tx, idx) => {
                const sourceCash = cashAccounts.find(a => a.id === tx.cashRegister);
                return (
                  <tr key={tx.id} className={idx % 2 === 1 ? 'bg-slate-50' : 'bg-white'}>
                    ...
                    <td className="p-2 border border-slate-200 text-center font-mono font-bold text-rose-800">{tx.transactionNumber}</td>
                    ...
                    <td className="p-2 border border-slate-200 text-center">{sourceCash?.name || 'صندوق نقدی'}</td>
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-black text-slate-900">لیست پرداخت‌ها (اسناد خروج از صندوق)</h1>
            <p className="text-xs sm:text-sm text-slate-500 font-medium">
              مشاهده تمامی مبالغ پرداختی به تأمین‌کنندگان، شرکت‌های حمل‌ونقل و تسویه بدهی‌های تجارتی
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          <button
            ...
            title="چاپ گزارش لیست فیلتر شده پرداخت‌ها"
          >
          ...
          {onCashTransfer && (
            <button
              id="btn-goto-cash-transfer-from-payments"
              type="button"
          ...
          <button
            id="btn-goto-new-payment"
            type="button"
            onClick={onNewPayment}
            className="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-xs font-bold transition shadow-md cursor-pointer shrink-0"
          >
            <Plus className="w-4 h-4" />
            <span>ثبت سند پرداخت جدید</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-2xs">
          <div className="flex items-center justify-between text-xs text-slate-500 font-bold mb-2">
            <span>مجموع پرداختی‌های افغانی {hasActiveFilters ? '(فیلتر شده)' : ''}</span>
            <TrendingDown className="w-4 h-4 text-rose-600" />
          </div>
          <div className="text-xl font-black text-rose-700 font-mono">
            {formatNumber(metrics.totalAFN)} <span className="text-xs text-rose-800">؋</span>
          </div>
          <div className="mt-2 text-[11px] text-slate-400 font-medium">
            کسر شده از صندوق‌های افغانی
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-2xs">
          <div className="flex items-center justify-between text-xs text-slate-500 font-bold mb-2">
            <span>مجموع پرداختی‌های دالری {hasActiveFilters ? '(فیلتر شده)' : ''}</span>
            <TrendingDown className="w-4 h-4 text-rose-600" />
          </div>
          <div className="text-xl font-black text-rose-700 font-mono">
            ${formatNumber(metrics.totalUSD)} <span className="text-xs text-rose-800">دالر</span>
          </div>
          <div className="mt-2 text-[11px] text-slate-400 font-medium">
            کسر شده از صندوق‌های دالری
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-2xs">
          <div className="flex items-center justify-between text-xs text-slate-500 font-bold mb-2">
            <span>تعداد اسناد پرداخت {hasActiveFilters ? `(${metrics.count} از ${metrics.allCount})` : ''}</span>
            <Wallet className="w-4 h-4 text-blue-600" />
          </div>
          <div className="text-xl font-black text-slate-900 font-mono">
            {metrics.count} <span className="text-xs text-slate-500">سند</span>
          </div>
          <div className="mt-2 text-[11px] text-slate-400 font-medium">
      ...
      <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-2xs space-y-3">
        <div className="flex flex-col md:flex-row items-center justify-between gap-3">
          ...
          <div className="relative w-full md:w-80">
            ...
            <input
              type="text"
              placeholder="جستجو با شماره سند، نام دریافت‌کننده یا شرح..."
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              className="w-full pl-3 pr-9 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-900 outline-none focus:bg-white focus:border-rose-500 transition"
            />
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 w-full md:w-auto justify-end">
            <div className="flex items-center bg-slate-100 p-1 rounded-xl text-xs font-bold text-slate-600">
              ...
              <button
                ...
                className={`px-3 py-1 rounded-lg transition ${currencyFilter === 'AFN' ? 'bg-white text-rose-700 shadow-2xs' : 'hover:text-slate-900'}`}
              >
                افغانی
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-lg transition ${currencyFilter === 'USD' ? 'bg-white text-rose-700 shadow-2xs' : 'hover:text-slate-900'}`}
              >
                دالر
              </button>
            </div>
          ...
          <div className="pt-3 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 text-xs">
            ...
            <div className="space-y-1 sm:col-span-2">
              <label className="text-[11px] font-bold text-slate-700 flex items-center gap-1">
                ...
                <span>بازه تاریخی اسناد پرداخت (شمسی):</span>
              </label>
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="w-1/2 px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-mono font-bold text-slate-800 focus:bg-white focus:border-rose-500 outline-none"
                />
              ...
              <div className="flex items-center gap-1 pt-1">
                ...
                <button
                  ...
                  className={`px-2 py-0.5 rounded text-[10px] font-bold ${datePreset === 'today' ? 'bg-rose-700 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                >
                  امروز
                </button>
                <button
                  ...
                  className={`px-2 py-0.5 rounded text-[10px] font-bold ${datePreset === '7days' ? 'bg-rose-700 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                >
                  ۷ روز اخیر
                </button>
                <button
                  ...
                  className={`px-2 py-0.5 rounded text-[10px] font-bold ${datePreset === '30days' ? 'bg-rose-700 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                >
                  یک ماه اخیر
                </button>
              </div>
            ...
            <div className="space-y-1">
              <label className="text-[11px] font-bold text-slate-700 flex items-center gap-1">
                ...
                <span>صندوق مبدأ پرداخت:</span>
              </label>
              <select
                value={selectedCashAccount}
                onChange={e => setSelectedCashAccount(e.target.value)}
                className="w-full px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold text-slate-800 focus:bg-white focus:border-rose-500 outline-none"
              >
            ...
            <div className="space-y-1">
              <label className="text-[11px] font-bold text-slate-700 flex items-center gap-1">
                ...
                <span>طرف حساب (فروشنده / طلبکار):</span>
              </label>
              <select
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="w-32 px-2.5 py-1 bg-slate-50 border border-slate-200 rounded-lg text-xs font-mono font-bold text-slate-800 focus:bg-white focus:border-rose-500 outline-none"
                  />
                ...
                {hasActiveFilters && (
                  <span className="text-[11px] text-rose-700 font-bold bg-rose-50 px-2.5 py-0.5 rounded border border-rose-200">
                    نمایش {filteredPayments.length} سند پرداخت از مجموع {payments.length} سند
                  </span>
                )}
              </div>
            </div>
          ...
          <table className="w-full text-right border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200 text-[11px] font-black text-slate-600">
                ...
                <th className="py-3 px-4">تأمین‌کننده / دریافت‌کننده</th>
                <th className="py-3 px-4">شرح سند</th>
                <th className="py-3 px-4">مبلغ کسر از طلبکاری</th>
                <th className="py-3 px-4">صندوق پرداختی</th>
              ...
              {filteredPayments.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-12 text-center text-slate-400 font-medium">
                    سند پرداختی با مشخصات انتخاب‌شده یافت نشد.
                  </td>
                </tr>
              ) : (
                    ...
                    <tr key={tx.id} className="hover:bg-slate-50/70 transition">
                      ...
                      <td className="py-3 px-4">
                        <span className="px-2.5 py-1 rounded-lg text-[11px] font-bold bg-slate-100 text-slate-800">
                          {sourceCash?.name || 'صندوق نقدی'}
                          {tx.isExchange && tx.cashAmount && (
                            <span className="text-rose-700 font-mono mr-1">
